...
result_t YDlidarDriver::waitPackage(node_info *node, uint32_t timeout) {
  ...
  if (CheckSunResult) {
    if (m_intensities) {
      (*node).sync_quality = (uint8_t((package.packageSample[package_Sample_Index].PakageSampleDistance &
                                       0x03) << (8 - LIDAR_RESP_MEASUREMENT_DISTANCE_SHIFT)) ||
    ...
    if ((*node).distance_q2 != 0) {
      AngleCorrectForDistance = (int32_t)(((atan(((21.8 * (155.3 - ((*node).distance_q2))) / 155.3) / ((
                                              *node).distance_q2))) * 180.0 / 3.1415) * 64.0);
    } else {
